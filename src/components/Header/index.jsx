  ...
  if (inPurchase) {
    return (
      <header className="d-flex justify-content-between">
        <NavLink onClick={setInPurchaseFunc} to="/api/products">
          <h3>GAME-STORE</h3>
        </NavLink>
      </header>
    );
  ...
  return (
    <header className="d-flex justify-content-between pt-2">
      ...
      <div className="basketContainer">
        <button
          type="button"
          className="btnBasket btn btn-outline-success"
        ...
        {isVisible && (
          <div className="basket">
            <ul className="list-group">{basketElements}</ul>
            {basket.length !== 0 && (
              <div className="text-center">Total: {totalPrice}$</div>
            )}
            <div className="d-flex justify-content-center mb-3">
              {basket.length !== 0 && (
                <NavLink to="/api/purchase">
                  <button
                    className="btn btn-success me-3"
                    onClick={() => setInPurchase(true)}
                  >
                    Buy
                  </button>
                </NavLink>
